...
export default function TicketMusicalpage() { // 뮤지컬 페이지 (장르 코드 : GGGA)
    ...
    const fetchData = async (sDate, eDate) => {
      try {
        ...
        const allResponse = await axios.get("http://localhost:8080/tl/getPerformanceInfo", {
          params: { startdate: sDate, enddate: eDate, cpage: 1, rows: 5, shcate: "GGGA" },
        });
    ...
    const fetchRegionData = async (sDate, eDate) => {
      try {
        ...
        const regionResponse = await axios.get("http://localhost:8080/tl/getPerformanceInfo", {
          params: { startdate: sDate, enddate: eDate, cpage: 1, rows: 5, signgucode: regionCode, shcate: "GGGA" },
        });
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", margin: "20px" }}>
        <h2>전체 공연</h2>
        <Link to="/ticket/list" state={{ genre : "GGGA" }} // 모두 보기 버튼
          style={{ margin: "5px", border: "1px, solid, black", }}
        >
          모두 보기
        </Link>
      ...
      <div style={{ display: "flex", alignItems: "center", margin: "20px" }}>
        <h2>지역별 공연</h2>
        <Link to="/ticket/list" state={{ region: regionCode, genre : "GGGA" }} // 모두 보기 버튼
          style={{ margin: "5px", border: "1px, solid, black", }}
        >
          모두 보기
        </Link>

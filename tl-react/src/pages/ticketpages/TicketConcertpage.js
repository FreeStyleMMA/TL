    ...
    const fetchStatusData = async (sDate, eDate) => {
      try {
        ...
        const rankResponse = await axios.get("http://localhost:8080/tl/getPerformanceInfo", {
          params: { startdate: sDate, enddate: eDate, rows: 5, cpage: 1, shcate: "CCCD", perRequestT: "rankConcert" },
        });
        const recommendResponse = await axios.get("http://localhost:8080/tl/getPerformanceInfo", {
          params: { startdate: sDate, enddate: eDate, rows: 5, cpage: 1, shcate: "CCCD", perRequestT: "recommend" },
        });
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", margin: "20px" }}>
        <h2>공연 랭킹</h2>
        <Link to="/ticket/list" state={{ type: "rankConcert", genre : "CCCD" }} // 모두 보기 버튼

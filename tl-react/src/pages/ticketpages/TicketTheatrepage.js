...
export default function TicketTheatrepage() { // 연극 페이지(뮤지컬 코드 : AAAA)
    ...
    const fetchData = async () => {
      try {
        ...
        const allResponse = await axios.get('http://localhost:8080/tl/getPerformanceInfo', {
          params: { startdate: startDate, enddate: endDate, cpage: 1, rows: 5, shcate: "AAAA"}
        });
        // 공연 랭킹
        const rankResponse = await axios.get('http://localhost:8080/tl/getPerformanceInfo', {
          params: { startdate: startDate, enddate: endDate, requestType: "rank", shcate: "AAAA" }
        });
        // 추천 공연
        const recommendResponse = await axios.get('http://localhost:8080/tl/getPerformanceInfo', {
          params: { startdate: startDate, enddate: endDate, requestType: "recommend", shcate: "AAAA" }
        });
        console.log(allResponse.data);
        setPerformanceInfos(allResponse.data); // 연극 전체 공연 데이터 저장
        console.log(rankResponse.data);
        setRankPerformanceInfos(rankResponse.data); // 연극 공연 랭킹 데이터 저장
        console.log(recommendResponse.data);
        setRecommendPerformanceInfos(recommendResponse.data); // 연극 추천 공연 데이터 저장
  ...
  useEffect(() => {
    const fetchRegionData = async () => {
      try {
        ...
        setRegionPerformanceInfos(regionResponse.data); // 지역별 연극 공연 데이터 저장
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex" }}>
        {performanceInfos.map((performanceInfo, idx) => ( //전체 연극 공연 정보 출력
          <div key={idx} style={{width:"120px", height:"160px", ml:"15px", mr:"15px"}}>
            <Link to="/ticket/info" state={{ performanceInfo }} style={{ display: "flex", flexDirection:"column", alignItems:"center" }}>
              ...
              <h6>{performanceInfo.per_title}</h6> {/* 연극 공연 제목 */}
            </Link>
          </div>
        ))}
      </div>
      <h2>공연 랭킹</h2>
      <div style={{ display: "flex" }}>
      {rankPerformanceInfos.map((performanceInfo, idx) => ( // 연극 공연 랭킹 정보 출력
          <div key={idx} style={{width:"120px", height:"160px", ml:"15px", mr:"15px"}}>
            <Link to="/ticket/info" state={{ performanceInfo }} style={{ display: "flex", flexDirection:"column", alignItems:"center" }}>
              <img alt={performanceInfo.per_title} style={{ width: "100px", height: "120px" }} src={performanceInfo.per_poster} /> {/* 포스터 이미지 */}
              <h6>{performanceInfo.per_rank}등 : {performanceInfo.per_title}</h6> {/* 연극 공연 등수, 제목 */}
            </Link>
          </div>
        ))}
      </div>
      <h2>추천 공연</h2>
      <div style={{ display: "flex" }}>
      {recommendPerformanceInfos.map((performanceInfo, idx) => ( // 추천 연극 공연 정보 출력
          <div key={idx} style={{width:"120px", height:"160px", ml:"15px", mr:"15px"}}>
            <Link to="/ticket/info" state={{ performanceInfo }} style={{ display: "flex", flexDirection:"column", alignItems:"center" }}>
              <img alt={performanceInfo.per_title} style={{ width: "100px", height: "120px" }} src={performanceInfo.per_poster} /> {/* 포스터 이미지 */}

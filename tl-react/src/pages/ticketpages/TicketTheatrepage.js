    ...
    const fetchData = async (sDate, eDate) => {
      try {
        ...
        const allResponse = await axios.get("http://localhost:8080/tl/getPerformanceInfo", {
          params: { startdate: sDate, enddate: eDate, cpage: 1, rows: 5, shcate: "AAAA" },
        });
    ...
    const fetchRegionData = async (sDate, eDate) => {
      try {
        ...
        const regionResponse = await axios.get("http://localhost:8080/tl/getPerformanceInfo", {
          params: { startdate: sDate, enddate: eDate, cpage: 1, rows: 5, signgucode: regionCode, shcate: "AAAA" },
        });
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", margin: "20px" }}>
        <h2>공연 랭킹</h2>
        <Link to="/ticket/list" state={{ type: "rankTheatre", genre: "AAAA" }} // 모두 보기 버튼

...
public class PerformanceRequestDto { // api에서 공연 목록을 받아올 때 요청할 매개변수를 담을 클래스
	private String perRequestD = LocalDate.now().format(DateTimeFormatter.BASIC_ISO_DATE); // 요청 날짜(오늘 날짜)
    private String startdate = "";     // 공연 시작일자 (필수, 8자리, 예: 20230101)
    private String enddate = "";       // 공연 종료일자 (필수, 8자리, 예: 20230630)
    private String cpage = "";         // 현재 페이지 (필수, 최대 3자리, 예: 1)
    private String rows = "";          // 페이지당 목록 수 (필수, 최대 3자리, 최대 100건, 예: 10)
    ...
    private String perRequestT = "";   // 추천 공연 여부
}
